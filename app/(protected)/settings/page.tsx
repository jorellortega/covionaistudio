...
import { LogOut, User, Key, Save, Settings, Bot, Lock, Unlock, Shield, Sparkles, Cog, CreditCard, Zap, RefreshCw, Mail } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  // Load profile information from users table
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const supabase = getSupabaseClient()
      const { error } = await supabase.auth.resetPasswordForEmail(profileEmail.trim(), {
      ...
      toast({
        ...
        description: `Password reset email has been sent to ${profileEmail.trim()}. Please check your inbox.`,
      })
    } catch (error: any) {
  ...
  // Update profile name and email in users table
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    ...
                    disabled={
                      ...
                      !profileName.trim() || 
                      !profileEmail.trim() ||
                      (profileName.trim() === profileData?.name && profileEmail.trim() === profileData?.email)
                    }
                    className="flex items-center gap-2"
                  >
                    {isSavingProfile ? (
        ...
        {/* Admin Section - Only visible to CEOs */}
        {userRole === 'ceo' && !isLoadingRole && (
          <>
