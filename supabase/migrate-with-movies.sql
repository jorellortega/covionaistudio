-- Cinema Studio Database Setup with Movie Support
-- Run this in your Supabase SQL Editor
-- This will create all necessary tables, functions, and policies including movie support
...
-- Create projects table with movie support
CREATE TABLE IF NOT EXISTS public.projects (
  ...
  project_type TEXT DEFAULT 'general' CHECK (project_type IN ('general', 'movie', 'video', 'animation')),
  genre TEXT,
  scenes INTEGER DEFAULT 0,
  duration TEXT,
  thumbnail TEXT,
  movie_status TEXT DEFAULT 'Pre-Production' CHECK (movie_status IN ('Pre-Production', 'Production', 'Post-Production', 'Distribution')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
BEGIN
  ...
  RAISE NOTICE '🎬 Movie support added to projects table';
  RAISE NOTICE '🔒 Row Level Security enabled on all tables';
  RAISE NOTICE '🔑 Authentication triggers configured';
  RAISE NOTICE '📝 You can now start creating movies!';
END $$;
